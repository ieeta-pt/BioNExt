...
def run_cells(testset, output_file, dataset_folder, kb_folder):
    
    print("load training data and kbases for cells")
    ...
    for doc in training_data:
        for entity in doc["entities"]:
            if entity["label"] == "CellLine":
                training_direct_match[transform_entity(entity["text"])].append(entity["linked_id"])
    training_lookup = build_direct_lookup_function(training_direct_match)

    device = "cuda"

    embeddings= {}
    embeddings_id = {}
    _path = f"{kb_folder}/Cellosaurus/"
    ...
    # run direct match taxonomy match
    ...
    test_run_data_dict = {doc["id"]:{str(ent["id"]):ent for ent in doc["entities"] if ent["label"] == "CellLine"} for doc in test_run}
    # build doc
    ...
    print("number of predicted cell:", c,c2)
