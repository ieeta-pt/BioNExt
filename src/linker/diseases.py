...
def run_diseases(testset, output_file, dataset_folder, kb_folder):
    
    print("load training data and kbases for diseases")
    ...
    for doc in training_data:
        for entity in doc["entities"]:
            if entity["label"] == "DiseaseOrPhenotypicFeature" or entity["label"] == "Disease":
    ...
    embeddings= {}
    embeddings_id = {}
    _path = f"{kb_folder}/CTD-diseases/"
    ...
    test_run_data_dict = {doc["id"]:{str(ent["id"]):ent for ent in doc["entities"]  if ent["label"] == "DiseaseOrPhenotypicFeature" or ent["label"] == "Disease"} for doc in test_run}
    ...
    print("number of predicted diseases:", c)
