...
def load_train_test_split(
    ...
    # for i, row in pd.read_csv(os.path.join(folder_path,"tsv", "medprocner_tsv_train_subtask1.tsv"), sep="\t").iterrows():
    #     docs[row["filename"]].append({k:row[k] for k in ["ann_id", "label", "start_span", "end_span", "text"]})
    # #dict of filename:[annotaions]
    # data = [{"filename":os.path.join(folder_path,"txt",f"{k}.txt"), "annotations":v} for k,v in docs.items()]

    # random.shuffle(clean_data)
    # split_index = int(len(clean_data) * test_split_percentage)
